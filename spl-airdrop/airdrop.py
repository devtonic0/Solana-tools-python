...
with open(output_file, "w") as f, tqdm(total=len(amount_wallet_list)) as pbar:
    for index, (amount, wallet_id) in enumerate(amount_wallet_list, start=1):
        cmd = f"spl-token transfer YoTRanE8uaaA9wnfuV5AXXciq6vGjAvye7WLcS2ADp4 {amount} {wallet_id} --fund-recipient --allow-unfunded-recipient"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            success_msg = f"\033[32m\u2714 Transferred {amount} WL to {wallet_id}\033[0m"
            pbar.write(success_msg)
            f.write(success_msg + "\n")
        else:
            error_msg = f"\033[31mError transferring WL to {wallet_id}: {result.stderr}\033[0m"
